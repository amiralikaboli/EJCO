...
int main() {
    ...
    for (int iter = 0; iter < 1 + 5; ++iter) {
        ...
        unordered_multimap<int, int> interm6_trie0(interm6_offsets.size());
        ...
        for (const auto &ci_off : ci_offsets) {
            ...
            auto chn_range = chn_trie0.equal_range(x0);
            if (chn_range.first != chn_range.second) {
                ...
                if (rt_trie0.contains(x1)) {
                    ...
                    auto interm4_range = interm4_trie0.equal_range(x2);
                    if (interm4_range.first != interm4_range.second) {
                        auto x3 = ci_person_id[ci_off];
                        auto interm6_range = interm6_trie0.equal_range(x3);
                        if (interm6_range.first != interm6_range.second) {
                            ...
                            for (auto interm6_it = interm6_range.first; interm6_it != interm6_range.second; ++interm6_it) {
                                auto interm6_off = interm6_it->second;
