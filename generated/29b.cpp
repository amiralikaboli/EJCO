...
int main() {
	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		auto pi_trie0 = emhash6::HashMap<int, emhash6::HashMap<int, bool>>();
		...
		auto it3_trie0 = emhash6::HashMap<int, bool>();
		...
		for (const auto &[ci_trie1, _, x0]: ci_trie0) {
			if (chn_trie0.contains(x0)) {
				...
				for (const auto &[ci_trie2, _, x1]: ci_trie1) {
					if (rt_trie0.contains(x1)) {
						auto &rt_trie1 = rt_trie0.at(x1);
						for (const auto &[pi_trie1, _, x2]: pi_trie0) {
