...
int main() {
	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		build_trie(ci_trie0, ci_role_id, ci_person_role_id, ci_person_id, ci_movie_id);
		...
		vector<tuple<int, int, int, int, int, int, string, string>> res;
						...
						for (const auto &[x2, n_trie1]: n_trie0) {
							if (an_trie0.contains(x2)) {
								...
								if (ci_trie2.contains(x2)) {
									...
									for (const auto &[x4, mi_trie1]: mi_trie0) {
																							...
																							for (const auto &mc_off: mc_trie2) {
																								for (const auto &it_off: it_trie1) {
																									for (const auto &cn_off: cn_trie1) {
																										res.push_back({x0, x1, x2, x4, x5, x6, n_name[n_off], t_title[t_off]});
																									}
																								}
																							}
																						}
