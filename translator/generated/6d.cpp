									...
									for (const auto &mk_off: mk_trie2) {
										for (const auto &t_off: t_trie1) {
											for (const auto &k_off: k_trie1) {
												res.push_back({x0, x1, n_name[n_off], x2, t_title[t_off], k_keyword[k_off]});
											}
										}
									}
								}
