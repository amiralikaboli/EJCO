...
int main() {
	...
	for (int iter = 0; iter < 1 + 10; ++iter) {
		...
		timer.Reset();
		auto t_vtrie0 = phmap::flat_hash_map<int, vector<int>>(t_offsets.size());
		auto t_itrie0 = phmap::flat_hash_map<int, int>(t_offsets.size());
		build_trie(t_vtrie0, t_itrie0, t_offsets, t_id);
