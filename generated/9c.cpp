...
int main() {
	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		auto chn_trie0 = emhash6::HashMap<int, vector<int>>();
		build_trie(chn_trie0, chn_id);
		auto an_trie0 = emhash6::HashMap<int, vector<int>>();
		...
		for (const auto &[ci_trie1, _, x0]: ci_trie0) {
			if (n_trie0.contains(x0)) {
				...
				for (const auto &[mc_trie1, _, x1]: mc_trie0) {
								...
								for (const auto &[ci_trie3, _, x3]: ci_trie2) {
									if (rt_trie0.contains(x3)) {
										...
										if (t_trie0.contains(x2)) {
											...
											for (const auto &[ci_trie4, _, x5]: ci_trie3) {
