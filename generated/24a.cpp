					...
					if (ci_trie0.contains(x1)) {
						...
						for (const auto &[ci_trie2, _, x2]: ci_trie1) {
							if (n_trie0.contains(x2)) {
								...
								if (mi_trie0.contains(x1) && t_trie0.contains(x1) && mc_trie0.contains(x1)) {
									auto &mi_trie1 = mi_trie0.at(x1);
									auto &t_trie1 = t_trie0.at(x1);
									auto &mc_trie1 = mc_trie0.at(x1);
									for (const auto &[mc_trie2, _, x4]: mc_trie1) {
										if (cn_trie0.contains(x4)) {
											auto &cn_trie1 = cn_trie0.at(x4);
											for (const auto &[ci_trie3, _, x5]: ci_trie2) {
												if (chn_trie0.contains(x5)) {
													auto &chn_trie1 = chn_trie0.at(x5);
													if (an_trie0.contains(x2)) {
														auto &an_trie1 = an_trie0.at(x2);
														for (const auto &[ci_trie4, _, x7]: ci_trie3) {
															if (rt_trie0.contains(x7)) {
																auto &rt_trie1 = rt_trie0.at(x7);
																for (const auto &[mi_trie2, _, x8]: mi_trie1) {
																	if (it_trie0.contains(x8)) {
																		auto &it_trie1 = it_trie0.at(x8);
