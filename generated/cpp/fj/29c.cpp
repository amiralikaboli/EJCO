	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		for (int ci_i = ci_trie0.first; ci_i <= ci_trie0.second; ++ci_i) {
			...
			if (rt_trie1.first != -1) {
				...
				auto interm4_trie1 = find_range(interm4_offsets, interm4_col1, x1, interm4_trie0);
				if (interm4_trie1.first != -1) {
					auto x2 = ci_person_id[ci_off];
					auto interm6_trie1 = find_range(interm6_offsets, interm6_col0, x2, interm6_trie0);
