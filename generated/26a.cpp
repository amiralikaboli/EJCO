...
int main() {
	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		auto ci_trie0 = emhash6::HashMap<int, emhash6::HashMap<int, emhash6::HashMap<int, bool>>>();
		build_trie_bool(ci_trie0, ci_movie_id, ci_person_role_id, ci_person_id);
		...
		for (const auto &[k_trie1, _, x0]: k_trie0) {
			if (mk_trie0.contains(x0)) {
				...
				for (const auto &[mk_trie2, _, x1]: mk_trie1) {
					if (t_trie0.contains(x1) && mi_idx_trie0.contains(x1) && cc_trie0.contains(x1) && ci_trie0.contains(x1)) {
							...
							if (chn_trie0.contains(x2)) {
								...
								for (const auto &[ci_trie3, _, x3]: ci_trie2) {
									if (n_trie0.contains(x3)) {
										...
										for (const auto &[t_trie2, _, x4]: t_trie1) {
											if (kt_trie0.contains(x4)) {
												...
												for (const auto &[mi_idx_trie2, _, x5]: mi_idx_trie1) {
													if (it2_trie0.contains(x5)) {
														...
														for (const auto &[cc_trie2, _, x6]: cc_trie1) {
															if (cct1_trie0.contains(x6)) {
																auto &cct1_trie1 = cct1_trie0.at(x6);
																for (const auto &[cc_trie3, _, x7]: cc_trie2) {
		...
		if (z == 0)
			cout << mn_n_name << " | " << mn_chn_name << " | " << mn_t_title << " | " << mn_mi_idx_info << endl;
		cout << "*" << " " << flush;
	}
	cout << endl;
