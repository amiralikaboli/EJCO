					...
					if (kt_trie0.contains(x1)) {
						...
						for (const auto &[x2, mi_idx_trie2]: mi_idx_trie1) {
							if (it2_trie0.contains(x2)) {
								...
								for (const auto &[x3, mc_trie1]: mc_trie0) {
									if (cn_trie0.contains(x3)) {
										...
										for (const auto &[x4, mc_trie2]: mc_trie1) {
											if (ct_trie0.contains(x4)) {
												...
												if (mc_trie2.contains(x0)) {
													auto &mc_trie3 = mc_trie2.at(x0);
													for (const auto &[x6, mk_trie1]: mk_trie0) {
														if (k_trie0.contains(x6)) {
															auto &k_trie1 = k_trie0.at(x6);
															if (mk_trie1.contains(x0)) {
																auto &mk_trie2 = mk_trie1.at(x0);
																for (const auto &[x8, mi_trie1]: mi_trie0) {
																	if (it1_trie0.contains(x8)) {
																		auto &it1_trie1 = it1_trie0.at(x8);
																		if (mi_trie1.contains(x0)) {
																			auto &mi_trie2 = mi_trie1.at(x0);
