...
int main() {
	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		build_trie(ci_trie0, ci_role_id, ci_movie_id, ci_person_role_id);
					...
					if (t_trie0.contains(x1)) {
						...
						for (const auto &[x2, mc_trie2]: mc_trie1) {
							if (ct_trie0.contains(x2)) {
								...
								for (const auto &[x3, mc_trie3]: mc_trie2) {
																	...
																	for (const auto &ct_off: ct_trie1) {
																		for (const auto &cn_off: cn_trie1) {
																			for (const auto &chn_off: chn_trie1) {
																				res.push_back({x0, x1, x2, x3, x5, t_title[t_off], chn_name[chn_off]});
																			}
																		}
																	}
																}
