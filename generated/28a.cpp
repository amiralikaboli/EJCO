		...
		for (const auto &[k_trie1, _, x0]: k_trie0) {
			if (mk_trie0.contains(x0)) {
				...
				for (const auto &[mk_trie2, _, x1]: mk_trie1) {
					if (cc_trie0.contains(x1) && mc_trie0.contains(x1) && t_trie0.contains(x1) && mi_idx_trie0.contains(x1)) {
												...
												if (ct_trie0.contains(x5)) {
													...
													for (const auto &[t_trie2, _, x6]: t_trie1) {
														if (kt_trie0.contains(x6)) {
															...
															for (const auto &[mi_idx_trie2, _, x7]: mi_idx_trie1) {
																if (it2_trie0.contains(x7)) {
																	auto &it2_trie1 = it2_trie0.at(x7);
																	for (const auto &[cc_trie2, _, x8]: cc_trie1) {
																		if (cct1_trie0.contains(x8)) {
																			auto &cct1_trie1 = cct1_trie0.at(x8);
																			for (const auto &[cc_trie3, _, x9]: cc_trie2) {
