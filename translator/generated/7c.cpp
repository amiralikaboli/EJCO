...
int main() {
	...
	vector<tuple<int, int, int, string, int, string>> res;
	for (const auto &[x0, ci_trie1]: ci_trie0) {
		if (t_trie0.contains(x0) && ml_trie0.contains(x0)) {
			auto &t_trie1 = t_trie0.at(x0);
			auto &ml_trie1 = ml_trie0.at(x0);
			for (const auto &[x1, ci_trie2]: ci_trie1) {
				if (pi_trie0.contains(x1) && an_trie0.contains(x1) && n_trie0.contains(x1)) {
					auto &pi_trie1 = pi_trie0.at(x1);
														...
														for (const auto &pi_off: pi_trie2) {
															for (const auto &it_off: it_trie1) {
																for (const auto &an_off: an_trie1) {
																	res.push_back({x0, x1, x2, n_name[n_off], x3, pi_info[pi_off]});
																}
															}
														}
													}
