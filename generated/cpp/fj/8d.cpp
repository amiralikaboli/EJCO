...
int main() {
	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		pair<int, int> an1_trie0 = {0, an1_offsets.size() - 1};
		build_trie(rt_offsets, rt_id);
		build_trie(an1_offsets, an1_person_id);
		...
		for (int ci_i = ci_trie0.first; ci_i <= ci_trie0.second; ++ci_i) {
			...
			if (rt_trie1.first != -1) {
				...
				auto an1_trie1 = find_range(an1_offsets, an1_person_id, x1, an1_trie0);
				if (an1_trie1.first != -1) {
					...
					if (interm0_trie1.first != -1 && t_trie1.first != -1) {
						...
						if (n1_trie1.first != -1) {
							...
							for (int an1_i = an1_trie1.first; an1_i <= an1_trie1.second; ++an1_i) {
