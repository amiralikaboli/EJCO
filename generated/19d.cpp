...
int main() {
	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		auto ci_trie0 = emhash6::HashMap<int, emhash6::HashMap<int, emhash6::HashMap<int, emhash6::HashMap<int, bool>>>>();
		build_trie_bool(ci_trie0, ci_movie_id, ci_person_id, ci_person_role_id, ci_role_id);
		auto rt_trie0 = emhash6::HashMap<int, bool>();
		build_trie_bool(rt_trie0, rt_id);
		auto chn_trie0 = emhash6::HashMap<int, bool>();
		build_trie_bool(chn_trie0, chn_id);
		auto n_trie0 = emhash6::HashMap<int, vector<int>>();
		build_trie(n_trie0, n_id);
		auto an_trie0 = emhash6::HashMap<int, bool>();
		...
		auto mc_trie0 = emhash6::HashMap<int, emhash6::HashMap<int, bool>>();
			...
			if (t_trie0.contains(x0) && mc_trie0.contains(x0)) {
				...
				for (const auto &[mc_trie2, _, x1]: mc_trie1) {
					if (cn_trie0.contains(x1)) {
						...
						for (const auto &[ci_trie2, _, x2]: ci_trie1) {
							if (n_trie0.contains(x2)) {
								auto &n_trie1 = n_trie0.at(x2);
								for (const auto &[ci_trie3, _, x3]: ci_trie2) {
									if (chn_trie0.contains(x3)) {
										...
										if (an_trie0.contains(x2)) {
											...
											if (mi_trie0.contains(x0)) {
												...
												for (const auto &[rt_trie1, _, x6]: rt_trie0) {
													if (ci_trie3.contains(x6)) {
														auto &ci_trie4 = ci_trie3.at(x6);
														for (const auto &[it_trie1, _, x7]: it_trie0) {
															if (mi_trie1.contains(x7)) {
																auto &mi_trie2 = mi_trie1.at(x7);
