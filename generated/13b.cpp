...
int main() {
	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		vector<tuple<int, int, int, int, int, int, string, string, string>> res;
						...
						if (kt_trie0.contains(x2)) {
							...
							for (const auto &[x3, mi_trie2]: mi_trie1) {
								if (it2_trie0.contains(x3)) {
									...
									for (const auto &[x4, miidx_trie2]: miidx_trie1) {
																					...
																					for (const auto &it_off: it_trie1) {
																						for (const auto &ct_off: ct_trie1) {
																							for (const auto &cn_off: cn_trie1) {
																								res.push_back({x0, x2, x3, x4, x5, x6, t_title[t_off], miidx_info[miidx_off], cn_name[cn_off]});
																							}
																						}
																					}
																				}
