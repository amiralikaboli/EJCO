...
int main() {
	...
	for (int z = 0; z < 1 + 5; ++z) {
		...
		vector<tuple<int, int, int, int, int, int, int, int, string, string, string, string>> res;
					...
					if (chn_trie0.contains(x1)) {
						...
						for (const auto &[x2, mk_trie1]: mk_trie0) {
							if (k_trie0.contains(x2)) {
								...
								for (const auto &[x3, t_trie1]: t_trie0) {
									if (kt_trie0.contains(x3)) {
										auto &kt_trie1 = kt_trie0.at(x3);
										for (const auto &[x4, mi_idx_trie1]: mi_idx_trie0) {
											if (it2_trie0.contains(x4)) {
												...
												for (const auto &[x5, cc_trie1]: cc_trie0) {
													if (cct1_trie0.contains(x5)) {
														...
														for (const auto &[x6, cc_trie2]: cc_trie1) {
															if (cct2_trie0.contains(x6)) {
																auto &cct2_trie1 = cct2_trie0.at(x6);
																for (const auto &[x7, mi_idx_trie2]: mi_idx_trie1) {
																	if (cc_trie2.contains(x7)) {
																		auto &cc_trie3 = cc_trie2.at(x7);
																		if (t_trie1.contains(x7)) {
																			auto &t_trie2 = t_trie1.at(x7);
																			if (mk_trie1.contains(x7)) {
																														...
																														for (const auto &cc_off: cc_trie3) {
																															for (const auto &cct1_off: cct1_trie1) {
																																for (const auto &cct2_off: cct2_trie1) {
																																	res.push_back({x0, x1, x2, x3, x4, x5, x6, x7, n_name[n_off], chn_name[chn_off], t_title[t_off], mi_idx_info[mi_idx_off]});
																																}
																															}
																														}
																													}
