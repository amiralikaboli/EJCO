	...
	def _generate_node(
		...
		for idx, eq_cols in enumerate(compiled_plan):
			...
			if join_attrs_order[rel_0][col_0] == idx:
				...
				yield f"sum(<{self.var_mng.x_var(idx)}, {self.var_mng.next_trie_var(rel_0)}> in {self.var_mng.trie_var(rel_0)}) {{\n"
			...
			for rel, col in eq_cols[start_offset:]:
				conditions.append(f"{self.var_mng.x_var(join_attrs_order[rel][col])} in {self.var_mng.trie_var(rel)}")
			yield f"if ({' && '.join(conditions)}) {{\n"
			self.indent += 1

			for rel, col in eq_cols[start_offset:]:
				yield f"let {self.var_mng.next_trie_var(rel)} = {self.var_mng.trie_var(rel)}({self.var_mng.x_var(join_attrs_order[rel][col])})\n"
		...
		if self.var_mng.is_root_rel(interm_rel):
			for rel, _, proj_cols in build_plan:
				for col in proj_cols:
					yield f"let {self.var_mng.mn_var(rel, col)} = sum(<{self.var_mng.offset_var(rel)}, _> in {self.var_mng.trie_var(rel)}) {{ {self.var_mng.rel_col_var(rel, col)}[{self.var_mng.offset_var(rel)}] }}\n"
		else:
			...
			for rel, cols in interm_rel2cols.items():
				yield f"sum(<{self.var_mng.offset_var(rel)}, _> in {self.var_mng.trie_var(rel)}) {{\n"
				self.indent += 1
			final_attrs = []
			for idx, (rel, col) in interm_cols:
				final_attrs.append(f"{self.var_mng.rel_col_var(rel, col)}[{self.var_mng.offset_var(rel)}]")
